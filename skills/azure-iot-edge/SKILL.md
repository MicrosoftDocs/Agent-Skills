---
name: azure-iot-edge
description: Provides expert guidance on Azure IoT Edge architecture, configuration, security, deployment, integrations, limits, troubleshooting, and decision-making for edge gateway and module-based solutions. Use this skill when designing or operating IoT Edge gateways, choosing provisioning and platform options (including EFLOW), planning CI/CD and automatic deployments, securing devices and downstream connectivity, diagnosing runtime/networking issues, or validating scalability and quota constraints. Not for generic Azure IoT Hub usage without IoT Edge, other Azure IoT services (e.g., IoT Central, DPS alone), or non-Azure edge runtimes.
...
| Decision Making | L53-L59 | Guidance on choosing IoT Edge device platforms, provisioning methods, EFLOW networking, and nested virtualization options for different deployment and infrastructure needs. |
...
| Security | L71-L83 | Configuring IoT Edge security: X.509 and EST certs, auth for downstream devices, confidential computing modules, Linux/Windows security principles, and securing traffic with Private Link. |
| Configuration | L84-L119 | Configuring IoT Edge devices/modules: networking, gateways, DPS provisioning (TPM/X.509/keys), EFLOW/VM setup, GPU, storage, metrics/monitoring, and deployment options. |

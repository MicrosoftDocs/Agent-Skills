...
| Decision Making | L151-L207 | Guidance for choosing Cosmos DB/Cosmos for PostgreSQL/MongoDB options, sizing, consistency, backup, cost, and detailed migration/upgrade paths from Cassandra, DynamoDB, MongoDB, and older SDKs. |
| Architecture & Design Patterns | L208-L251 | Architectural patterns for AI agents, multitenancy, HTAP, change feed, vector/semantic search, and high availability using Azure Cosmos DB and Cosmos DB for PostgreSQL. |
| Limits & Quotas | L252-L292 | Limits, quotas, and behaviors for Cosmos DB (all APIs and modes): throughput, autoscale, burst, backup/PITR, free tier, indexing, partitions, caching, fleets, and PostgreSQL cluster constraints. |
| Security | L293-L359 | Securing Cosmos DB and related services: identity/RBAC, keys & encryption, network isolation (VNet, Private Link, firewalls), TLS, auditing, policies, and data protection features. |
| Configuration | L360-L484 | Configuring and managing Cosmos DB and related services: throughput, indexing, backup/restore, global distribution, emulators, SDK tuning, monitoring, and deployment via Bicep/ARM/Terraform. |
| Integrations & Coding Patterns | [integrations.md](integrations.md) | Patterns and code samples for integrating Cosmos DB with apps, SDKs, drivers, Kafka/Spark/BI tools, change feed, vector search, and migrations across all Cosmos DB APIs. |
...
| Design focused vector search with Sharded DiskANN in Cosmos DB | https://learn.microsoft.com/en-us/azure/cosmos-db/gen-ai/sharded-diskann |
...
| Supported PostgreSQL engine versions in Cosmos DB for PostgreSQL | https://learn.microsoft.com/en-us/azure/cosmos-db/postgresql/reference-versions |
...
| Control Azure Cosmos DB throughput with buckets | https://learn.microsoft.com/en-us/azure/cosmos-db/throughput-buckets |
...
| Use and manage PostgreSQL extensions in Cosmos DB for PostgreSQL | https://learn.microsoft.com/en-us/azure/cosmos-db/postgresql/reference-extensions |

...
|----------|-------|-------------|
| Troubleshooting | L36-L45 | Debugging APIM policies and requests, handling policy errors, diagnosing platform issues (SNAT/timeouts, Key Vault certs), and using Diagnose and Solve for common APIM problems |
| Best Practices | L46-L57 | Scaling, throttling, security hardening (OWASP), policy fragments, SSE setup, testing and running self-hosted gateways/portals, and applying transformation/throttling policies. |
| Decision Making | L58-L69 | Guidance on APIM scaling, pricing tiers, cost management, DevOps/CI-CD planning, VNet choices, monetization features, and migrating from Amazon API Gateway |
| Architecture & Design Patterns | L70-L80 | Architectural patterns for placing APIM behind gateways (App Gateway, Front Door), integrating with AKS microservices, and designing reusable, coordinated policy/fragment pipelines and shared caching. |
| Limits & Quotas | L81-L98 | Policies and limits for API Management: rate/quotas per key, concurrency, Azure OpenAI/LLM token limits, import restrictions, and validating requests/responses against API schemas. |
| Security | L99-L132 | Securing API Management and portals: authN/Z with Entra ID/B2C/OAuth/JWT, client certs, TLS/ciphers, RBAC, managed identity, CORS, DDoS/Defender, LLM safety, and compliance policies. |
| Configuration | L133-L232 | Configuring Azure API Management behavior: policies, caching (response, key, semantic), networking/VNet, domains, logging/metrics, GraphQL/gRPC/WebSocket, workspaces, and self-hosted gateway settings. |
| Integrations & Coding Patterns | L233-L266 | Patterns and examples for integrating APIM with LLMs, MCP, SAP, Dapr, Service Bus/Event Hubs/Event Grid, logging/monitoring, portals, and external backends via policies and exports |
| Deployment | L267-L286 | Deploying APIM across regions and networks, self-hosted gateways (K8s, Docker, Arc, AKS, Container Apps), DR/backup/restore, soft-delete recovery, and developer portal deployment options. |
...
| Implement unit and end-to-end tests for self-hosted API Management developer portal | https://learn.microsoft.com/en-us/azure/api-management/developer-portal-testing |
| Manage context variables in API Management policy fragments | https://learn.microsoft.com/en-us/azure/api-management/fragment-variable-management |
...
| Implement shared metadata caching across API Management fragments | https://learn.microsoft.com/en-us/azure/api-management/fragment-metadata-cache |
| Design modular policy fragment pipelines in API Management | https://learn.microsoft.com/en-us/azure/api-management/fragment-pipeline-architecture |
| Coordinate policy execution using API Management fragments | https://learn.microsoft.com/en-us/azure/api-management/fragment-policy-coordination |
...
| Create and manage Azure API Management policy fragments | https://learn.microsoft.com/en-us/azure/api-management/policy-fragments |

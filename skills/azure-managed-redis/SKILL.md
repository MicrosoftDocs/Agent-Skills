...
description: Provides expert guidance on Azure Managed Redis troubleshooting, best practices, decision making, security, configuration, integrations, and deployment, including client usage, scaling, geo-replication, persistence, monitoring, and secure connectivity from .NET, Go, Node.js/TypeScript, and Python. Use this skill when planning or operating Azure Managed Redis, diagnosing connectivity/latency/timeouts, optimizing cost and performance, configuring Entra ID, Private Link, TLS, or automating deployments via ARM/Bicep, CLI, or PowerShell. Not for Azure Cache for Redis (classic PaaS), non-Azure Redis services, or general Redis OSS administration outside Azure Managed Redis.
...
|----------|-------|-------------|
| Troubleshooting | L35-L45 | Diagnosing and fixing Azure Managed Redis issues: connectivity, latency/timeouts, data loss, server resources, monitoring errors, and client-side performance using tools like redis-cli. |

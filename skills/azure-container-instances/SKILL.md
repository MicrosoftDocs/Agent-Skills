...
| Limits & Quotas | L60-L66 | ACI CPU/memory limits, regional quota checks, standby pool caps, and VNet-related deployment constraints and planning for container instances |
| Security | L67-L81 | Securing Azure Container Instances: TLS/HTTPS setup, image and secret protection, managed identities, RBAC, DNS safety, ACR auth, CMK encryption, and Azure Policy enforcement. |
| Configuration | L82-L115 | Configuring ACI container groups: YAML, env vars, DNS, volumes, probes, GPUs, restart/entrypoint, standby pools, networking, lifecycle, logging, and monitoring. |
| Deployment | L116-L119 | Using GitHub Actions to build, push, and automatically deploy container images to Azure Container Instances, including workflow YAML setup and authentication configuration. |

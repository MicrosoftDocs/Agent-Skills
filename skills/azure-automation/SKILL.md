...
| Best Practices | L43-L56 | Patterns and guidance for designing, structuring, error‑handling, and managing Azure Automation runbooks/DSC (modules, child runbooks, watcher tasks, DR, compliance remediation). |
...
| Security | L74-L92 | Securing Azure Automation: identities (system/user-assigned, Entra ID), RBAC, encryption, private endpoints, STIG/policy compliance, and security best practices for runbooks and accounts |
| Configuration | L93-L119 | Configuring Azure Automation accounts: runbooks, State Configuration/DSC, Hybrid Runbook Workers, networking, logging, policies, assets (credentials, certs, variables), packages, schedules, and source control. |
| Integrations & Coding Patterns | L120-L132 | Patterns for integrating runbooks with AWS, Azure alerts, webhooks, ARM/Office 365/SQL, email (SendGrid), and using SDKs/managed identity to automate external services. |
| Deployment | L133-L136 | Using Azure Automation State Configuration with Chocolatey to set up continuous deployment of software/configurations across Windows machines |

...
|----------|-------|-------------|
| Troubleshooting | L37-L50 | Diagnosing and fixing Azure SQL and SQL Server on Azure VM issues: connectivity, capacity, performance (I/O, memory, redo lag), transaction logs, geo-replication, and IaaS Agent extension errors. |
...
| Configuration | L105-L146 | Configuring SQL Server on Azure VMs: HA/DR (AGs, FCIs, listeners, load balancers), storage/perf, backups, editions/versions, and SQL IaaS Agent/monitoring setup. |
...
| Deployment | L153-L170 | Deploying and configuring SQL Server Always On availability groups and FCIs on Azure VMs, including single/multi-subnet, cross-region, prerequisites, and migration of VMs and storage. |

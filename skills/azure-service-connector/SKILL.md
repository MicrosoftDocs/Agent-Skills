...
|----------|-------|-------------|
| Troubleshooting | L30-L34 | Diagnosing and resolving Azure Service Connector errors and failures, including common error codes, connection issues, and steps to debug and fix failed connections. |
| Limits & Quotas | L35-L39 | Known limitations of Azure Service Connector, unsupported scenarios, and recommended workarounds or alternatives for connecting to services. |
| Security | L40-L46 | Security and auth for Service Connector: required permissions, Entra RBAC roles it assigns, and how to configure connection authentication options. |
...
| Integrations & Coding Patterns | L53-L83 | How to connect Azure apps to data, messaging, AI, and storage services (Cosmos DB, SQL, Kafka, Redis, Key Vault, Azure AI/OpenAI, MongoDB Atlas, Blob, queues, etc.) using Service Connector. |
| Deployment | L84-L88 | Region availability for Service Connector by compute type, and how to provision Service Connector connections using infrastructure-as-code tools (Bicep, ARM, Terraform, etc.). |

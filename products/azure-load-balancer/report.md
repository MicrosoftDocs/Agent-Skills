...
category_descriptions:
  ...
  decision-making: Guidance on choosing the right Load Balancer SKU, upgrading from
    Basic to Standard, and migrating workloads from AWS Network Load Balancer to Azure.
  best-practices: Designing and deploying Azure Load Balancer and VM scale set architectures
    for reliability, performance, scaling, health probes, and high availability best
    practices.
  limits-quotas: Limits, behaviors, and configuration of Load Balancer TCP idle timeouts,
    TCP resets, and outbound SNAT connection quotas for inbound and outbound traffic.
  troubleshooting: 'Diagnosing and fixing Azure Load Balancer issues: health event
    logs, monitoring/alerts, connectivity problems, and testing public frontend reachability.'
...
| [Load balancer best practices](https://learn.microsoft.com/en-us/azure/load-balancer/load-balancer-best-practices) | best-practices | 0.80 | The page explicitly focuses on best practices for deploying and configuring Azure Load Balancer, derived from platform experience and customer scenarios. These are product-specific DO/DON'T recommendations and configuration guidance that go beyond generic load balancing theory, fitting the best-practices sub-skill. While the summary doesn’t show exact numbers, such pages typically include concrete Azure-specific recommendations and gotchas. |
...
| [Standard Load Balancer metrics and diagnostics](https://learn.microsoft.com/en-us/azure/load-balancer/load-balancer-standard-diagnostics) | troubleshooting | 0.70 | The page focuses on using Azure Load Balancer diagnostics (Azure Monitor metrics, alerts, and Resource Health) specifically to monitor availability and troubleshoot inbound/outbound connectivity. This is product-specific troubleshooting guidance organized around diagnosing connectivity issues using particular diagnostic capabilities, which qualifies as expert troubleshooting knowledge beyond generic monitoring concepts. |
...
| [Upgrade options and guidance](https://learn.microsoft.com/en-us/azure/load-balancer/load-balancer-basic-upgrade-guidance) | decision-making | 0.70 | The page provides guidance for migrating from Basic to Standard Load Balancer, including planning, scripts, manual steps, and outbound connectivity considerations. It references key differences between SKUs and helps users decide how and when to upgrade, which aligns with decision-making around SKU selection and migration paths. This is product-specific upgrade guidance rather than a generic overview. |
...
| [CLI](https://learn.microsoft.com/en-us/azure/load-balancer/quickstart-load-balancer-standard-internal-cli) | 0.20 | Similar to index 1, this is a quickstart for creating an internal load balancer using Azure CLI. It is a basic tutorial without detailed limits, quotas, decision matrices, or product-specific troubleshooting or configuration tables, so it doesn’t meet the expert-knowledge criteria. |
| [CLI](https://learn.microsoft.com/en-us/azure/load-balancer/quickstart-load-balancer-standard-public-cli) | 0.20 | This is a quickstart tutorial for creating a public load balancer via Azure CLI. It focuses on step-by-step resource creation (VMs, Bastion, NAT Gateway, VNet) rather than limits, best-practices, troubleshooting, or configuration matrices. It doesn’t indicate detailed expert-only parameters, limits, or error mappings. |

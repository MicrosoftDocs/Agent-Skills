...
category_descriptions:
  ...
  configuration: 'How to configure Azure Bastion features: deployment options, Kerberos
    auth, monitoring/metrics/logs, native client access, session management, recordings,
    and shareable links.'
  limits-quotas: Configuring Azure Bastion host scaling limits, including max concurrent
    sessions, connection thresholds, and how to adjust capacity for performance and
    cost.
...
skill_description: Expert knowledge for Azure Bastion development including troubleshooting,
  ...
  Bastion for AKS private clusters, VM scale sets, native SSH/RDP clients, Kerberos
  auth, or hub/spoke VNets, and other Azure Bastion related development tasks. Not
  for Azure Virtual Network (use azure-virtual-network), Azure Virtual Machines (use
  azure-virtual-machines), Azure VPN Gateway (use azure-vpn-gateway), Azure Firewall
  (use azure-firewall).
use_when: Use when configuring Bastion for AKS private clusters, VM scale sets, native
  SSH/RDP clients, Kerberos auth, or hub/spoke VNets, and other Azure Bastion related
  development tasks.
...
| [Configure host scaling](https://learn.microsoft.com/en-us/azure/bastion/configure-host-scaling) | limits-quotas | 0.70 | Host scaling for Bastion Standard SKU is a product-specific capacity feature that typically documents how many concurrent connections each scale unit supports, SKU-specific constraints, and disruption behavior when changing units. These are concrete, numeric limits and behavioral constraints that qualify as expert knowledge beyond generic scaling concepts. |

...
category_descriptions:
  ...
  configuration: 'Configuring Azure Bastion behavior: scaling, auth (Kerberos), monitoring/metrics/logs,
...
skill_description: Expert knowledge for Azure Bastion development including troubleshooting,
  ...
  and integrations & coding patterns. Use when connecting via SSH/RDP to AKS private
  clusters or VMSS, using native clients, Kerberos auth, or IP-based Bastion, and
...
use_when: Use when connecting via SSH/RDP to AKS private clusters or VMSS, using native
  clients, Kerberos auth, or IP-based Bastion, and other Azure Bastion related development
  tasks.
...
| [Bastion configuration settings](https://learn.microsoft.com/en-us/azure/bastion/configuration-settings) | configuration | 0.75 | The article describes Azure Bastion resources and configuration settings. Given its focus on 'available configuration settings,' it likely enumerates specific setting names, options, and behaviors that are product-specific and not just conceptual. That aligns with the configuration sub-skill, as it helps an agent know concrete Bastion settings and how they are used. |
...
| [Configure host scaling](https://learn.microsoft.com/en-us/azure/bastion/configure-host-scaling) | configuration | 0.70 | Host scaling for Bastion Standard SKU is a product-specific configuration feature. The article is about adjusting the number of Bastion instances (scale units) and includes SKU requirements and behavioral constraints (e.g., changes disrupt active connections, plan during maintenance windows). This is detailed, product-specific configuration guidance rather than generic scaling theory, fitting the configuration sub-skill. |

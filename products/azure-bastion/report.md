...
| [RDP connection](https://learn.microsoft.com/en-us/azure/bastion/bastion-connect-vm-linux-rdp) | 0.20 | Primarily a step-by-step tutorial for connecting to a Linux VM via RDP using Azure Bastion. It focuses on portal-based connection steps and basic prerequisites (like xrdp) rather than detailed configuration matrices, limits, or troubleshooting mappings, so it does not meet the expert-knowledge criteria for any sub-skill type. |
| [SSH connection](https://learn.microsoft.com/en-us/azure/bastion/bastion-connect-vm-ssh-windows) | 0.20 | Primarily a step-by-step tutorial for connecting to a Windows VM via SSH using Azure Bastion through the portal. It does not emphasize configuration parameter tables, limits, error-code-based troubleshooting, or product-specific best-practice guidance with quantified impact; it’s mainly procedural how-to content. |

...
category_descriptions:
  configuration: 'Configuring VM Scale Sets: scaling rules, upgrades, health/repairs,
    networking, disks, images, autoscale, instance mix, standby pools, and management
    via portal, CLI, PowerShell, and ARM.'
...
skill_description: Expert knowledge for Azure Virtual Machine Scale Sets development
  ...
  Use when configuring autoscale rules, instance mix/Spot, standby pools, upgrade
  modes, or disk encryption via CLI/ARM, and other Azure Virtual Machine Scale Sets
  ...
  (use azure-container-instances), Azure Red Hat OpenShift (use azure-redhat-openshift).
use_when: Use when configuring autoscale rules, instance mix/Spot, standby pools,
  upgrade modes, or disk encryption via CLI/ARM, and other Azure Virtual Machine Scale
...
confusable_not_for: Not for Azure Virtual Machines (use azure-virtual-machines), Azure
  ...
  azure-service-fabric), Azure App Service (use azure-app-service).
confusable_slugs:
...
| [Extension sequencing](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/disk-encryption-extension-sequencing) | configuration | 0.70 | Article is about Azure Disk Encryption and VM Scale Set extension sequencing, which involves specific extension names, ordering, and configuration properties unique to this product. That fits configuration (how to set up extensions and their sequence) rather than general concepts. |
...
| [Use Azure Resource Manager templates](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/disk-encryption-azure-resource-manager) | configuration | 0.70 | Quickstart shows concrete Azure Disk Encryption configuration in ARM templates for VM Scale Sets (specific extension names, properties, and required parameters). This is product-specific configuration detail beyond generic knowledge, but not focused on limits, troubleshooting, or architecture. |
...
| [Azure Disk Encryption](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/disk-encryption-overview) | configuration | 0.65 | Provides instructions and required settings to enable Azure Disk Encryption for VM Scale Sets, including product-specific parameters and configuration steps. It is primarily configuration guidance, not just conceptual overview. |

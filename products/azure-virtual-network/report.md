...
category_descriptions:
  ...
  deployment: Planning and policy guidance for using MANA (multi-accelerated network
    adapters) with existing Azure VM sizes and managing MANA usage for NVA (network
    virtual appliance) workloads.
...
skill_description: Expert knowledge for Azure Virtual Network development including
  ...
  configuring NSGs/DNS/IPs, using MANA or NVAs, setting up VPN/NAT/LB, or securing
  VNet traffic, and other Azure Virtual Network related development tasks. Not for
...
use_when: Use when designing VNets, configuring NSGs/DNS/IPs, using MANA or NVAs,
  setting up VPN/NAT/LB, or securing VNet traffic, and other Azure Virtual Network
...
| [MANA on Network Virtual Appliances (NVAs) for existing VM sizes](https://learn.microsoft.com/en-us/azure/virtual-network/accelerated-networking-mana-network-virtual-appliance-opt-out) | deployment | 0.70 | Page focuses on how MANA behaves for Network Virtual Appliances on existing VM sizes and how to control/opt out of MANA deployments using Azure Policy to ensure performance. This includes product-specific behavior and policy-based control patterns for a particular hardware feature (MANA) on specific VM types, which is expert, deployment-focused knowledge not generally known from pretraining. It’s not generic troubleshooting or best practices, but concrete guidance on where and how MANA is applied for NVA deployments. |
...
| [MANA support for existing VM sizes](https://learn.microsoft.com/en-us/azure/virtual-network/accelerated-networking-mana-existing-sizes) | deployment | 0.68 | Page is an update describing which existing General Purpose Compute VM sizes and hardware generations support the Microsoft Azure Network Adapter (MANA) and how that affects Accelerated Networking. This is deployment- and platform-specific behavior tied to particular VM families/hardware that an LLM would not reliably know from training. It’s not just conceptual; it provides concrete, SKU-specific support details that guide how and where to deploy workloads using MANA. |

...
category_descriptions:
  configuration: 'Configuring Extended Zones access: registering subscriptions, requesting
    ...
    usage.'
  security: Encrypting Azure Extended Zone disks with customer-managed keys using
    Azure Key Vault, including setup, configuration steps, and security considerations
...
skill_description: Provides expert guidance on configuring, securing, and optimizing
  costs for Azure Extended Zones, including subscription registration, zone access
  requests, Azure Policy governance, disk encryption with customer-managed keys via
  Azure Key Vault, and decisions on Reserved Instances or Savings Plans. Use this
  skill when planning or managing workloads in Azure Extended Zones, setting up compliant
  and secure access, or evaluating and purchasing cost-optimization options specific
  to these zones. Not for general Azure region management, non-Extended Zone services,
  or unrelated networking, security, or cost tools outside Azure Extended Zones.
---
...
| [Encrypt disks with customer-managed keys in an Azure Extended Zone](https://learn.microsoft.com/en-us/azure/extended-zones/key-vault-encrypt-azure-extended-zone-disk) | security | 0.68 | The article describes product-specific security configuration for encrypting managed disks in Azure Extended Zones using customer-managed keys, Azure Key Vault, and Disk Encryption Sets, including the constraint that assigning a Disk Encryption Set to Extended Zone workloads is currently only supported via Azure CLI. This is detailed, version-specific behavior that an LLM is unlikely to know from training and fits the security sub-skill focused on configuration of encryption and key management. |

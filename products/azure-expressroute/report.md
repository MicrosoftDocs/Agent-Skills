...
category_descriptions:
  ...
  best-practices: 'Best practices for ExpressRoute design and operations: bandwidth
    upgrades, BGP community use, fixing asymmetric routing, QoS for Skype voice, and
    migrating to new gateways.'
  ...
  architecture-patterns: Designing resilient, multi-site, and DR-ready ExpressRoute
    topologies, routing optimization, high availability, VPN coexistence/backup, and
  ...
  integrations: Automating ExpressRoute circuit creation/management with PowerShell
    ...
    peering.
skill_description: Provides expert guidance on Azure ExpressRoute design, deployment,
  configuration, security, integrations, limits, and troubleshooting, including routing
  optimization, high availability patterns, VPN coexistence, bandwidth and gateway
  selection, and automation with PowerShell/CLI/Terraform. Use this skill when planning
  or modifying ExpressRoute circuits/gateways, resolving connectivity or performance
  issues, implementing DR-ready topologies, enforcing security controls (IPsec/MACsec/RBAC/NAT),
  or validating that your design stays within supported limits and quotas. Not for
  generic Azure networking, public VPN-only scenarios, or non-ExpressRoute connectivity
  products (e.g., Azure VPN Gateway without ExpressRoute, Azure Virtual WAN, or third-party
  SD-WAN design).
---
...
| [Routing requirements](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-routing) | configuration | 0.78 | A routing requirements page for ExpressRoute typically includes product-specific BGP and routing configuration details (required/allowed prefixes, ASNs, communities, routing policies, and constraints) that go beyond generic networking knowledge. These are concrete configuration requirements unique to ExpressRoute rather than conceptual guidance, fitting the configuration sub-skill. |
...
| [Asymmetric routing](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-asymmetric-routing) | best-practices | 0.70 | Explains ExpressRoute-specific asymmetric routing problems with stateful devices and provides concrete guidance on how to avoid or remediate dropped traffic in multi-path scenarios. These are product- and scenario-specific gotchas and recommendations rather than generic networking theory. |
...
| [Design for high availability](https://learn.microsoft.com/en-us/azure/expressroute/designing-for-high-availability-with-expressroute) | architecture-patterns | 0.70 | Contains ExpressRoute-specific high-availability architecture recommendations (how to avoid single points of failure across customer, provider, and Microsoft segments, and how to structure circuits and paths). This is actionable, product-specific architecture guidance beyond generic HA theory. |
...
| [Monitoring data reference](https://learn.microsoft.com/en-us/azure/expressroute/monitor-expressroute-reference) | configuration | 0.70 | A 'monitoring data reference' article typically enumerates all metrics, dimensions, and log categories for ExpressRoute in Azure Monitor, including metric names, units, and dimensions. This is product-specific configuration/reference data that LLMs are unlikely to know from training and fits the configuration sub-skill type best. |
...
| [Overview](https://learn.microsoft.com/en-us/azure/expressroute/design-architecture-for-resiliency) | architecture-patterns | 0.70 | Provides product-specific resiliency patterns for ExpressRoute (for example, recommended circuit/topology patterns, how to architect for site and path redundancy, and when to use each pattern). This is concrete architecture guidance unique to ExpressRoute rather than generic HA concepts. |
...
| [Evaluate ExpressRoute circuit resiliency](https://learn.microsoft.com/en-us/azure/expressroute/evaluate-circuit-resiliency) | architecture-patterns | 0.65 | Describes how to validate and test failover behavior for multi-site redundant ExpressRoute circuits, including how routes should be advertised and how resilient topologies behave. This is ExpressRoute-specific resiliency/architecture validation guidance, not just conceptual overview. |
...
| [Azure PowerShell](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-howto-circuit-arm) | 0.20 | Duplicate of index 2: a PowerShell-based ExpressRoute circuit quickstart. As with index 2, it is a basic operational tutorial and does not clearly contain expert-level configuration tables, limits, or troubleshooting matrices. |
| [Azure portal](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-howto-circuit-portal-resource-manager) | 0.20 | Duplicate of index 1: a portal-based ExpressRoute circuit quickstart. As with index 1, it appears to be a standard step-by-step guide without detailed expert-only configuration, limits, or decision criteria. |
| [Create an ExpressRoute circuit - Portal](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-howto-circuit-portal-resource-manager) | 0.20 | Quickstart for creating/modifying ExpressRoute circuits in the portal is primarily a step-by-step tutorial. The summary indicates basic creation, status checking, and lifecycle operations, without evidence of detailed limits, configuration parameter tables, or decision matrices. This is standard how-to content, not expert-knowledge configuration or limits. |
| [Create an ExpressRoute circuit - PowerShell](https://learn.microsoft.com/en-us/azure/expressroute/expressroute-howto-circuit-arm) | 0.20 | Quickstart for creating/modifying ExpressRoute circuits using PowerShell is a procedural guide. The summary focuses on basic operations (create, verify, update, delete) via cmdlets, with no indication of parameter tables, quotas, or specialized troubleshooting/decision content. This is generic tutorial content rather than expert configuration or limits. |
| [Monitor ExpressRoute](https://learn.microsoft.com/en-us/azure/expressroute/monitor-expressroute) | 0.20 | High-level guidance on monitoring ExpressRoute with Azure Monitor; appears to be an overview/start-here article linking to other resources, without clear indication of detailed limits, config tables, error codes, or product-specific parameters. |

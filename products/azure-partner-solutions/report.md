...
category_descriptions:
  ...
  security: Managing security for partner solutions, including Confluent Cloud RBAC
    ...
    deployments
  troubleshooting: Diagnosing and fixing onboarding, integration, and runtime issues
    for partner services on Azure (Confluent, Airflow, Datadog, Dynatrace, Elastic,
    Informatica, MongoDB, Neon, New Relic, NGINX, Palo Alto).
...
skill_description: Expert knowledge for Azure Partner Solutions development including
  ...
  Cloud RBAC, Dynatrace APM trials, MongoDB Atlas, or Palo Alto Cloud NGFW, and other
  Azure Partner Solutions related development tasks. Not for Azure Industry (use azure-industry),
  Azure Microsoft Discovery (use azure-microsoft-discovery), Azure Security (use azure-security),
  Azure Defender For Cloud (use azure-defender-for-cloud).
use_when: Use when using Service Connector, Confluent Cloud RBAC, Dynatrace APM trials,
  MongoDB Atlas, or Palo Alto Cloud NGFW, and other Azure Partner Solutions related
  development tasks.
confusable_not_for: Not for Azure Industry (use azure-industry), Azure Microsoft Discovery
  (use azure-microsoft-discovery), Azure Security (use azure-security), Azure Defender
  For Cloud (use azure-defender-for-cloud).
---
...
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/datadog/faq) | troubleshooting | 0.70 | Datadog on Azure FAQ explicitly mentions troubleshooting along with billing and feature questions. Such pages commonly document specific error conditions, integration behaviors, and Azure-linked account nuances that are unique to this integration, fitting the troubleshooting category as expert knowledge. |
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/dynatrace/faq) | troubleshooting | 0.70 | The Dynatrace FAQ covers onboarding, linking environments, SSO, billing, data residency, free trial, and troubleshooting. These topics for a specific Azure Native service typically include concrete error scenarios, configuration gotchas, and resolution steps that go beyond generic docs, matching troubleshooting expert knowledge. |
...
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/astronomer/faq) | troubleshooting | 0.65 | FAQ for a specific Azure Native ISV service typically includes concrete, product-specific answers such as error messages, onboarding issues, billing behaviors, and operational constraints that are not generic Airflow knowledge. While also covering general questions, these FAQs usually embed symptom→cause→solution guidance and service-specific behaviors, which best align with troubleshooting. |
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/elastic/faq) | troubleshooting | 0.65 | Elastic integration FAQ for Azure Native services usually contains detailed answers about provisioning, metrics/logs wiring, permissions, and common failure modes. These are product- and platform-specific, often mapping symptoms to causes and fixes, which aligns best with troubleshooting. |
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/mongo-db/faq) | troubleshooting | 0.65 | FAQ pages for managed partner services typically include product-specific error messages, support boundaries, and configuration gotchas (for example, portal integration behaviors, resource provider nuances, and support channels) that go beyond generic concepts. These map to symptom→cause→solution style guidance unique to MongoDB Atlas as an Azure Native Integration. |
...
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/informatica/faq) | troubleshooting | 0.60 | FAQ for Informatica Intelligent Data Management Cloud as an Azure Native ISV Service will typically include specific onboarding, configuration, and runtime issues unique to this integration. Those concrete Q&A patterns (e.g., why a resource doesn’t appear, how billing behaves) are troubleshooting-oriented expert knowledge. |
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/neon/faq) | troubleshooting | 0.60 | An Azure Native Neon FAQ is likely to document Azure-specific behaviors, limitations, and integration issues (for example, provisioning failures, connectivity patterns, or portal/RP quirks) with concrete resolutions. That aligns best with troubleshooting-style expert knowledge rather than generic overview content. |
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/nginx/faq) | troubleshooting | 0.60 | NGINXaaS FAQ content for Azure Native Integrations typically covers concrete operational questions, error conditions, and configuration edge cases specific to the Azure-hosted service, which fits the troubleshooting category more than conceptual or marketing material. |
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/palo-alto/faq) | troubleshooting | 0.60 | Cloud NGFW FAQ for Azure generally includes deployment and configuration issues, error scenarios, and Azure-integration specifics (for example, networking or policy application problems) with corresponding guidance, matching the troubleshooting sub-skill definition. |
...
| [FAQ](https://learn.microsoft.com/en-us/azure/partner-solutions/apache-kafka-confluent-cloud/faq) | 0.40 | FAQ page for Confluent Cloud on Azure; while FAQs can contain expert details, the provided summary is generic and does not confirm presence of specific limits, configuration tables, or error-code-based troubleshooting. |
...
| [Astro](https://learn.microsoft.com/en-us/azure/partner-solutions/astronomer/) | 0.20 | High-level description of Apache Airflow on Astro as an Azure Native ISV Service; summary is marketing/overview style with no expert-level specifics. |
| [Confluent](https://learn.microsoft.com/en-us/azure/partner-solutions/apache-kafka-confluent-cloud/) | 0.20 | Landing page for Confluent Cloud on Azure; describes managed Kafka/Flink service but no explicit limits, configuration tables, or troubleshooting mappings. |
| [Create a Qumulo resource](https://learn.microsoft.com/en-us/azure/partner-solutions/qumulo/create) | 0.20 | This is a quickstart for creating a Qumulo Scalable File Service instance in the portal. It is primarily step-by-step setup guidance and high-level description of created entities, without detailed limits, configuration matrices, or other expert-only data. |
...
| [Datadog](https://learn.microsoft.com/en-us/azure/partner-solutions/datadog/) | 0.20 | Landing/overview page for Datadog Azure Native ISV integration; description is high-level without specific limits, configs, error codes, or decision matrices. |
| [Dynatrace](https://learn.microsoft.com/en-us/azure/partner-solutions/dynatrace/) | 0.20 | Landing/overview page for Azure Native Dynatrace Service; only conceptual description of monitoring experience, no detailed configuration, limits, or troubleshooting content. |
| [Elastic](https://learn.microsoft.com/en-us/azure/partner-solutions/elastic/) | 0.20 | Overview for Elastic with Azure; focuses on what the integration does (centralized analytics) without product-specific parameters, limits, or decision guidance. |
| [F5](https://learn.microsoft.com/en-us/azure/partner-solutions/nginx/) | 0.20 | Landing page for NGINXaaS Azure Native ISV Service; describes ability to create/manage resources but no specific settings, quotas, or diagnostic guidance. |
...
| [Informatica](https://learn.microsoft.com/en-us/azure/partner-solutions/informatica/) | 0.20 | Overview for Informatica Azure Native ISV Service; mentions creating/managing organizations but not specific configuration parameters, quotas, or error codes. |
| [Manage resources](https://learn.microsoft.com/en-us/azure/partner-solutions/qumulo/manage) | 0.20 | The article describes how to manage Azure Native Qumulo settings at a general level. Based on the summary, it appears to be procedural management guidance rather than detailed configuration tables, limits, or troubleshooting content with specific error codes. |
...
| [MongoDB Atlas](https://learn.microsoft.com/en-us/azure/partner-solutions/mongo-db/) | 0.20 | High-level MongoDB Atlas Azure Native integration description; summary is conceptual (managed document DB, vector search) without detailed expert configuration or limits. |
| [Neon Serverless Postgres](https://learn.microsoft.com/en-us/azure/partner-solutions/neon/) | 0.20 | Deprecation-focused overview for Neon Serverless Postgres integration; description does not indicate detailed limits, configuration matrices, or troubleshooting content. |
| [Palo Alto Network](https://learn.microsoft.com/en-us/azure/partner-solutions/palo-alto/) | 0.20 | Overview for Cloud NGFW by Palo Alto Networks on Azure; summary is marketplace/benefits oriented without detailed security role mappings, configs, or limits. |

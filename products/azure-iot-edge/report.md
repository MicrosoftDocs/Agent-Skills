...
category_descriptions:
  configuration: 'Configuring IoT Edge devices/modules: networking (EFLOW, NICs, proxies,
    gateways, nested), provisioning (DPS, TPM, X.509, keys), GPU, storage, metrics/monitoring,
    and deployment options.'
  security: 'Security setup for IoT Edge: certificates, X.509 provisioning, EST server,
    auth for downstream devices, confidential computing modules, and securing traffic
    with Private Link/endpoints.'
...
skill_description: Expert knowledge for Azure IoT Edge development including troubleshooting,
  ...
  configuring EFLOW, DPS/TPM/X.509 provisioning, gateways/nested edge, CI/CD module
  deployments, or downstream device gateways, and other Azure IoT Edge related development
...
use_when: Use when configuring EFLOW, DPS/TPM/X.509 provisioning, gateways/nested
  edge, CI/CD module deployments, or downstream device gateways, and other Azure IoT
  Edge related development tasks.
...
| [PowerShell functions](https://learn.microsoft.com/en-us/azure/iot-edge/reference-iot-edge-for-linux-on-windows-functions) | configuration | 0.78 | Reference page for Azure IoT Edge for Linux on Windows PowerShell functions is likely a parameter-level command reference (function names, parameters, allowed values, defaults, and behaviors) specific to EFLOW deployment/provisioning/status. This is product-specific configuration/command surface detail that an LLM wouldn't reliably know from training and fits the configuration category best among the available types. |
...
| [Manage device certificates](https://learn.microsoft.com/en-us/azure/iot-edge/how-to-manage-device-certificates) | security | 0.70 | Page is focused on production-grade certificate management for IoT Edge, including specific certificate roles (device CA, workload CA, root CA), trust chains, and product-specific configuration steps/requirements for secure connections between runtime, modules, and downstream devices. This is concrete, implementation-focused security guidance rather than conceptual overview. |

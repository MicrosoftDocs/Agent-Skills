...
category_descriptions:
  configuration: 'Configuring IoT Edge devices/modules: networking, gateways, DPS
    provisioning (TPM/X.509/keys), EFLOW/VM setup, GPU, storage, metrics/monitoring,
    and deployment options.'
  security: 'Configuring IoT Edge security: X.509 and EST certs, auth for downstream
    devices, confidential computing modules, Linux/Windows security principles, and
    securing traffic with Private Link.'
  decision-making: Guidance on choosing IoT Edge device platforms, provisioning methods,
    EFLOW networking, and nested virtualization options for different deployment and
...
skill_description: Provides expert guidance on Azure IoT Edge architecture, configuration,
  security, deployment, integrations, limits, troubleshooting, and decision-making
  for edge gateway and module-based solutions. Use this skill when designing or operating
  IoT Edge gateways, choosing provisioning and platform options (including EFLOW),
  planning CI/CD and automatic deployments, securing devices and downstream connectivity,
  diagnosing runtime/networking issues, or validating scalability and quota constraints.
  Not for generic Azure IoT Hub usage without IoT Edge, other Azure IoT services (e.g.,
  IoT Central, DPS alone), or non-Azure edge runtimes.
---
...
| [Create test certificates](https://learn.microsoft.com/en-us/azure/iot-edge/how-to-create-test-certificates) | security | 0.78 | Describes how to create and install test/demo certificates for IoT Edge devices, including the specific certificate types required for runtime, modules, and downstream devices. This is product-specific security configuration (certificate hierarchy, usage, and installation steps) that an LLM is unlikely to know from training. It aligns with the security sub-skill because it covers concrete certificate requirements and configuration for secure communication. |
| [IoT Edge agent and hub properties](https://learn.microsoft.com/en-us/azure/iot-edge/module-edgeagent-edgehub) | configuration | 0.78 | Described as providing desired and reported properties of the runtime modules; this typically includes property names, allowed values, and semantics, matching product-specific configuration reference. |
...
| [Understand and use createOptions](https://learn.microsoft.com/en-us/azure/iot-edge/how-to-use-create-options) | configuration | 0.78 | The page describes using the createOptions parameter in IoT Edge deployment manifests to configure Docker-compatible container settings (port mappings, memory limits, GPU options, host resource access). It focuses on specific configuration parameters and how to apply them at runtime for IoT Edge modules, which is product-specific configuration knowledge rather than generic container usage. |
...
| [VM on a DMZ](https://learn.microsoft.com/en-us/azure/iot-edge/how-to-configure-iot-edge-for-linux-on-windows-iiot-dmz) | configuration | 0.74 | How-to article for Azure IoT Edge for Linux on Windows in a DMZ with multiple NICs. This scenario requires product-specific VM and network configuration steps (e.g., binding EFLOW to specific NICs, routing between offline and online networks, Windows host and VM network settings). These are concrete configuration patterns unique to EFLOW rather than generic networking concepts, fitting the configuration sub-skill. |
...
| [Create an IoT Edge device](https://learn.microsoft.com/en-us/azure/iot-edge/how-to-create-iot-edge-device) | decision-making | 0.65 | Article explicitly aims to give a clear picture of platform, provisioning, and authentication options; this is guidance for selecting among options and solution patterns, fitting decision-making criteria. |
...
| [Create a hierarchy of IoT Edge devices](https://learn.microsoft.com/en-us/azure/iot-edge/tutorial-nested-iot-edge-for-linux-on-windows) | 0.20 | Tutorial on creating a hierarchy of IoT Edge for Linux on Windows devices (nested edge). It is primarily a step-by-step tutorial/architecture scenario without clear configuration parameter tables, limits, or product-specific troubleshooting mappings. It reads as guided setup rather than a reusable expert reference for limits, configuration matrices, or error diagnosis. |
...
| [Version history and release notes](https://learn.microsoft.com/en-us/azure/iot-edge/version-history) | 0.20 | Release history and notes; likely version timelines and lifecycle info but not limits, configs, or troubleshooting mappings as defined. |
...
| [Classify images with Custom Vision](https://learn.microsoft.com/en-us/azure/iot-edge/tutorial-deploy-custom-vision) | 0.10 | Tutorial for image classification at the edge with Custom Vision; scenario-focused walkthrough rather than detailed configuration, limits, or troubleshooting. |
| [Deploy a Stream Analytics module](https://learn.microsoft.com/en-us/azure/iot-edge/tutorial-deploy-stream-analytics) | 0.10 | Tutorial for deploying Azure Stream Analytics as an IoT Edge module; focuses on example job and deployment steps, not expert configuration or limits. |
...
| [Filter data with Azure Functions](https://learn.microsoft.com/en-us/azure/iot-edge/tutorial-deploy-function) | 0.10 | Step-by-step tutorial for deploying an Azure Function as an IoT Edge module; typical how-to content without config tables, limits, or error mappings. |

...
category_descriptions:
  security: 'Securing ADX clusters: auth/RBAC, managed identities, encryption (disk/CMK),
    network/private endpoints, Azure Policy, conditional access, and data privacy/purge
    controls.'
...
skill_description: Provides expert guidance on Azure Data Explorer (ADX) troubleshooting,
  best practices, decision making, architecture & design patterns, limits & quotas,
  security, configuration, integrations & coding patterns, and deployment. Use this
  skill when designing or operating ADX clusters, optimizing performance and cost,
  securing and networking environments, integrating with external tools and services,
  or diagnosing ingestion/query/cluster issues. It covers topics such as DR and multi-tenant
  patterns, schema and ingestion tuning, SKU and scaling choices, DevOps deployment,
  and quota behaviors. Not for generic Azure data services or other analytics engines
  like Synapse, Fabric, Databricks, or Elasticsearch.
---
...
| [Configure customer-managed keys](https://learn.microsoft.com/en-us/azure/data-explorer/customer-managed-keys) | security | 0.70 | Contains product-specific security configuration constraints: requirement to use Azure Key Vault, that the cluster and key vault must be in the same region but can be in different subscriptions, and guidance on key storage/creation. These are concrete, service-specific security settings. |
...
| [Manage cluster locks](https://learn.microsoft.com/en-us/azure/data-explorer/manage-cluster-locks) | security | 0.70 | Page gives product-specific security/management behavior for Azure Data Explorer cluster locks, including how Delete and Read-only locks interact with Azure Data Explorer RBAC and what operations are blocked. This is concrete, service-specific configuration and permission behavior that goes beyond generic resource lock concepts. |
...
| [Migrate VNet injected cluster to private endpoints](https://learn.microsoft.com/en-us/azure/data-explorer/security-network-migrate-vnet-to-private-endpoint) | deployment | 0.65 | Describes a product-specific migration path from Virtual Network Injection (with a concrete retirement date) to private endpoints, including that migration creates new engine and data management clusters and switches connections. This is deployment-focused expert guidance about a required migration and resulting deployment topology. |
...
| [Enable double encryption](https://learn.microsoft.com/en-us/azure/data-explorer/cluster-encryption-double) | 0.40 | Explains how to enable double encryption during cluster creation but appears to be a simple UI toggle description without detailed configuration parameters, ranges, or security role specifics. |
...
| [Create a private endpoint](https://learn.microsoft.com/en-us/azure/data-explorer/security-network-private-endpoint-create) | 0.30 | Step-by-step style article on creating a private endpoint; summary does not indicate detailed security configuration parameters, role names, or settings tables beyond generic private endpoint usage. |
...
| [Security overview](https://learn.microsoft.com/en-us/azure/data-explorer/security) | 0.30 | High-level introduction to Azure Data Explorer security; no specific RBAC role names, permission scopes, or concrete configuration parameters are evident in the summary. |

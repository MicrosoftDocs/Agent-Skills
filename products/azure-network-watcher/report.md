...
category_descriptions:
  configuration: 'Configuring and governing Network Watcher logging: Connection Monitor
    AMA setup, NSG/VNet flow logs, Traffic Analytics schemas, filters, templates,
...
| [VNet flow logs filtering](https://learn.microsoft.com/en-us/azure/network-watcher/vnet-flow-logs-filtering) | configuration | 0.70 | The article describes product-specific filtering options for Azure Network Watcher virtual network flow logs (e.g., filtering by flow state, action, IP ranges, ports, protocols). These are concrete configuration capabilities and parameters unique to this service, not just conceptual logging guidance, so it best fits the configuration sub-skill. It does not focus on limits, troubleshooting, or architecture patterns. |

...
category_descriptions:
  configuration: 'Configuring and governing Network Watcher logging: AMA setup, Connection
    Monitor schemas, NSG/VNet flow logs (enable, filter, templates), Traffic Analytics
    schemas, and Azure Policy enforcement.'
...
skill_description: Expert knowledge for Azure Network Watcher development including
  ...
  Analytics, Connection Monitor, packet capture, or Sentinel integrations, and other
...
use_when: Use when configuring NSG/VNet flow logs, Traffic Analytics, Connection Monitor,
  packet capture, or Sentinel integrations, and other Azure Network Watcher related
  development tasks.
...
| [VNet flow logs filtering](https://learn.microsoft.com/en-us/azure/network-watcher/vnet-flow-logs-filtering) | configuration | 0.68 | The page describes detailed, product-specific filtering options for Azure Network Watcher virtual network flow logs (for example, filtering by flow state, action, IP ranges, ports, and protocols). These are concrete configuration capabilities and parameters unique to this feature rather than generic logging concepts, fitting the configuration sub-skill. It is not primarily about limits, troubleshooting, or architecture. |

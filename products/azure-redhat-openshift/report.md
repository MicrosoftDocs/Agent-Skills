...
skill_description: Expert knowledge for Azure Red Hat OpenShift development including
  ...
  configuring networking/storage, securing with Entra/Key Vault, or integrating GPUs/virtualization,
...
use_when: Use when creating ARO clusters, configuring networking/storage, securing
  with Entra/Key Vault, or integrating GPUs/virtualization, and other Azure Red Hat
  OpenShift related development tasks.
...
| [Support policies for Azure Red Hat OpenShift 4](https://learn.microsoft.com/en-us/azure/openshift/support-policies-v4) | best-practices | 0.70 | Support policy pages for Azure Red Hat OpenShift typically enumerate specific allowed and disallowed configuration changes (for example, which internal components you may or may not modify, unsupported features like Technology Preview items, and other concrete 'do/don’t' rules). These are product-specific, actionable guidelines that affect supportability and are not generic knowledge, fitting the best-practices category. |
...
| [Set up OpenShift Virtualization](https://learn.microsoft.com/en-us/azure/openshift/howto-create-openshift-virtualization) | 0.20 | The summary describes what OpenShift Virtualization is and its high-level benefits for Azure Red Hat OpenShift. It reads as an overview/tutorial-style introduction without clear indication of detailed configuration tables, limits, error codes, or decision matrices. Lacking evidence of specific expert-only parameters or constraints, it does not meet the expert knowledge criteria. |

...
category_descriptions:
  ...
  deployment: Guides for signing and verifying container images with Notation in Azure
    ...
    vs Azure Government.
  security: 'Security hardening for Azure: threat modeling mitigations, AKS image
    signing, ransomware defense, incident response, data protection, access control,
  ...
  decision-making: Guidance on when and how to use customer-managed keys in Azure,
    comparing key management options (Key Vault, managed HSM, service-specific CMK
    support) and choosing the right approach.
skill_description: Provides expert guidance on Azure Security, including hardening
  best practices, secure configuration, deployment patterns, troubleshooting, and
  decision-making for key management and supply chain security. Use this skill when
  you need to design or review Azure security architectures, configure features like
  firewalls, antimalware, logging, CMK/Key Vault, or AKS protections, secure CI/CD
  (image signing, SBOMs), compare Azure vs Azure Government security options, or resolve
  Customer Lockbox issues. Not for general Azure administration, non-Azure clouds,
  or M365/Defender product questions.
---
...
| [Choosing a key management solution](https://learn.microsoft.com/en-us/azure/security/fundamentals/key-management-choose) | decision-making | 0.74 | Article is explicitly about selecting between Azure Key Vault, Managed HSM, Cloud HSM, and Payment HSM based on scenarios and requirements, including a flowchart and comparison-style guidance. This is product-specific technology selection and trade-off guidance, matching decision-making. |
...
| [Services supporting CMKs](https://learn.microsoft.com/en-us/azure/security/fundamentals/encryption-customer-managed-keys-support) | decision-making | 0.65 | Page enumerates which Azure services support CMKs via Key Vault or Managed HSM. This is concrete, product-specific capability mapping that informs decisions about which services can meet CMK requirements, aligning best with decision-making among the available services. |

...
category_descriptions:
  ...
  configuration: Configuring Batch pools, tasks, networking, containers, autoscale,
    monitoring/diagnostics, and storage mounts, plus reference for events, metrics,
    and management APIs
...
skill_description: Expert knowledge for Azure Batch development including troubleshooting,
  ...
  configuring Batch pools/tasks, autoscale, CI/CD deployments, Storage/App Insights
  integration, or render/MPI workloads, and other Azure Batch related development
...
use_when: Use when configuring Batch pools/tasks, autoscale, CI/CD deployments, Storage/App
  Insights integration, or render/MPI workloads, and other Azure Batch related development
  tasks.
...
| [Configure access to compute nodes](https://learn.microsoft.com/en-us/azure/batch/pool-endpoint-configuration) | configuration | 0.78 | The page describes how to configure node endpoints (RDP/SSH and other ports) on Azure Batch pool compute nodes, including product-specific behavior changes tied to API version 2024-07-01 and a cutoff date (30 November 2025) for automatic port mapping. This is detailed, product-specific configuration behavior that an LLM is unlikely to know from training. The content focuses on endpoint configuration parameters and version-dependent defaults, which aligns best with the configuration sub-skill. |

...
category_descriptions:
  ...
  decision-making: Guidance on choosing VPN Gateway SKUs, understanding SKU mappings,
    migrating P2S/SSTP and Classic gateways, and planning remote work scenarios using
    P2S VPN gateways
  ...
  deployment: 'Deploying and configuring Azure VPN Gateways: create/migrate gateways
    and public IP SKUs, switch active/active-active, set up S2S VPNs, and deploy VPN
    client profiles via Intune.'
...
skill_description: Provides expert guidance on Azure VPN Gateway architecture, configuration,
  security, deployment, integrations, limits, and troubleshooting, including S2S/P2S
  setup, BGP/IPsec design, SKU selection, and connectivity with on-premises and other
  clouds. Use this skill when planning or operating Azure VPN Gateway-based hybrid
  or remote access solutions, diagnosing connection or performance issues, hardening
  VPN security, or deciding between gateway SKUs and topologies for high availability
  and scale. Not for Azure Virtual Network Manager, Azure Virtual WAN, third-party
  VPN appliances, or non-Azure VPN products.
---
...
| [About gateway SKUs](https://learn.microsoft.com/en-us/azure/vpn-gateway/about-gateway-skus) | decision-making | 0.78 | SKU selection guidance for Azure VPN Gateway with product-specific criteria (types of connections, performance, features) that influence which SKU to choose. This is concrete decision guidance between tiers/SKUs, beyond generic concepts. |
...
| [About gateway SKU consolidation & migration](https://learn.microsoft.com/en-us/azure/vpn-gateway/gateway-sku-consolidation) | decision-making | 0.70 | Explains migration from older VPN Gateway SKUs to new zone-redundant SKUs, including which SKUs map to which and pricing/availability implications. This is SKU/tier selection and migration decision guidance. |
...
| [Migrate a Basic SKU public IP address to Standard](https://learn.microsoft.com/en-us/azure/vpn-gateway/basic-public-ip-migrate-howto) | deployment | 0.70 | Step-by-step migration of Basic to Standard public IP for specific VPN Gateway SKUs (VpnGw1–5), including behavior of IP retention and connectivity impact. This is a product-specific deployment/migration pattern tied to particular SKUs. |
...
| [About Basic SKU public IP address migration](https://learn.microsoft.com/en-us/azure/vpn-gateway/basic-public-ip-migrate-about) | deployment | 0.65 | Describes product-specific migration process and timelines for moving VPN Gateway deployments from Basic to Standard public IP SKUs, including SKU-dependent timelines and deployment-impact considerations, which are deployment/migration constraints. |
...
| [Remove the Basic SKU public IP reference - Basic SKU VPN gateways](https://learn.microsoft.com/en-us/azure/vpn-gateway/basic-sku-public-ip-remove) | deployment | 0.65 | Covers a specific deployment change for Basic SKU VPN gateways to remove Basic public IP references without downtime, tied to retirement of Basic public IPs. This is a product-specific deployment/migration operation. |
...
| [Upgrade a gateway SKU](https://learn.microsoft.com/en-us/azure/vpn-gateway/gateway-sku-upgrade) | 0.40 | Primarily a how-to upgrade guide (portal/PowerShell/CLI) with a rough downtime estimate; lacks detailed configuration matrices, limits, or SKU comparison criteria that would qualify as expert decision, configuration, or deployment knowledge. |
...
| [Support and troubleshooting](https://learn.microsoft.com/en-us/azure/vpn-gateway/vpn-gateway-support-help) | 0.10 | Support/help options page; does not contain product-specific limits, configuration parameters, error-code troubleshooting, or other expert technical details—primarily guidance on where to get assistance. |
